...
BACKUP_MODEL_ORDER = [
    ...
    InvoiceItem,
    LabOrderItem,
    # Keep report rows after their patient/order/item records so restores remain valid
    # when those logical references become database foreign keys in a later migration.
    LabReport,
    # A confirmed public booking can reference both a patient and appointment.
    PublicAppointmentBooking,
    # OTP challenges only contain a context reference and can be restored last.
    PortalOtpChallenge,
    ReturnItem,
    VendorPurchaseItem,
    VendorNote,
...
HEAVY_QUERY_INDEXES = [
    ...
    ("ix_lab_test_active_name", "lab_test", ["is_active", "name"]),
    ("ix_lab_order_created_at", "lab_order", ["created_at"]),
    ("ix_lab_order_patient_created", "lab_order", ["patient_id", "created_at"]),
    ("ix_lab_order_item_order_id", "lab_order_item", ["lab_order_id"]),
    ("ix_lab_report_order_status", "lab_report", ["lab_order_id", "status"]),
    ("ix_lab_report_mobile_status", "lab_report", ["mobile", "status"]),
    ("ix_lab_report_status_published", "lab_report", ["status", "published_at"]),
    ("ix_portal_otp_purpose_mobile_sent", "portal_otp_challenge", ["purpose", "mobile", "sent_at"]),
    ("ix_portal_otp_context_purpose", "portal_otp_challenge", ["context_ref", "purpose"]),
    (
        "ix_public_appointment_booking_date_type_status",
        "public_appointment_booking",
        ["appointment_date", "booking_type", "status"],
    ),
    ("ix_public_appointment_booking_mobile_created", "public_appointment_booking", ["mobile", "created_at"]),
    (
        "ix_public_appointment_booking_status_expiry",
        "public_appointment_booking",
